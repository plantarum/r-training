...
At this point, \texttt{lattice} has pretty much been displaced by \texttt{ggplot2}, although you may
like to check it out too.
...
scales used in each figure. So \texttt{grconvertX(0.9, from = "npc", to = "user")} returns the X
coordinate to plot our text 90\% of the way to the left side of the plot, regardless of the scale
used in that plot. With this addition, we have the following code, and the generated panels:
